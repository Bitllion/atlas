...
import { rememberLocation, rememberOperator, savedLocations, savedOperator } from '../api/inventory'
...
const locations = ref<InventoryLocation[]>(savedLocations())
...
async function createLocation() {
  ...
  rememberLocation(data); locations.value = savedLocations(); targetId.value = data.id; creating.value = false
}
...
onMounted(async () => { if (props.action === 'deploy') racks.value = (await objectApi.racks()).data.items })
</script>
...
<template>
  <div class="modal-mask" @click.self="emit('close')"><section class="modal card" role="dialog" aria-modal="true">
    ...
    <template v-if="action === 'stock'">
      ...
      <p v-if="!locations.length" class="inline-tip">后端暂未提供位置查询接口，请先在此新建位置。</p>
