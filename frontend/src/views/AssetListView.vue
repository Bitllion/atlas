...
import type { Asset, ObjectType } from '../types'

const router = useRouter()
const assets = ref<Asset[]>([]); const types = ref<ObjectType[]>([]); const loading = ref(false); const total = ref(0); const page = ref(1); const status = ref(''); const action = ref<'stock' | 'deploy' | null>(null); const selected = ref<Asset | null>(null)
const pageSize = 20
...
<template><section class="page">
  ...
  <div class="card table-card"><div v-if="loading" class="empty">正在加载…</div><div v-else-if="!assets.length" class="empty">暂无资产记录</div><div v-else class="table-scroll"><table><thead><tr><th>资产编号</th><th>关联设备名</th><th>类型</th><th>状态</th><th>所有者</th><th>位置</th><th>操作</th></tr></thead><tbody>
    <tr v-for="item in assets" :key="item.id" class="clickable" @click="router.push(`/assets/${item.id}`)"><td class="name-cell">{{ item.asset_number }}</td><td>{{ item.object.name }}</td><td>{{ typeMap[item.object.object_type_id] || '—' }}</td><td><span class="status" :class="item.lifecycle_status.toLowerCase()">{{ statusName[item.lifecycle_status] || item.lifecycle_status }}</span></td><td class="id-cell">{{ item.owner_org_id || '—' }}</td><td class="id-cell">{{ item.inventory_location_id || item.object.deployed_location_id || '—' }}</td><td class="actions" @click.stop><button v-if="item.lifecycle_status === 'RECEIVED'" class="link" @click="open(item, 'stock')">入库</button><button v-if="item.lifecycle_status === 'STOCK'" class="link" @click="open(item, 'deploy')">部署</button><RouterLink :to="`/assets/${item.id}`">详情</RouterLink></td></tr>
