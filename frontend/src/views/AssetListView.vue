...
const statusColor = (value: string) => ({ REQUESTED: 'blue', APPROVED: 'arcoblue', ORDERED: 'purple', PURCHASED: 'cyan', RECEIVED: 'orange', STOCK: 'orange', IN_TRANSIT: 'gold', DEPLOYING: 'orange', DEPLOYED: 'green', ACTIVE: 'green', MAINTENANCE: 'orange', TRANSFERRED: 'purple', RETIRED: 'red', RECOVERED: 'blue' }[value] || 'gray')
...
function reset() { status.value = ''; filter() }
...
<template><section class="page arco-page">
  ...
  <a-card class="arco-filter-card" :bordered="false"><a-form :model="{}" layout="inline" @submit.prevent="filter"><a-form-item label="生命周期状态"><a-select v-model="status" placeholder="全部状态" allow-clear><a-option value="">全部状态</a-option><a-option v-for="(name, value) in statusName" :key="value" :value="value">{{ name }}</a-option></a-select></a-form-item><a-form-item><a-space><a-button type="primary" html-type="submit"><template #icon><IconSearch /></template>查询</a-button><a-button @click="reset">重置</a-button></a-space></a-form-item></a-form></a-card>
