...
const statusColor = (value: ObjectStatus) => ({ PLANNED: 'blue', ACTIVE: 'green', INACTIVE: 'gray', MAINTENANCE: 'orange', RETIRED: 'gray' }[value] || 'gray')
...
async function confirmRemove() { if (!pendingDelete.value) return; deleting.value = true; try { await objectApi.remove(pendingDelete.value.id); deleteVisible.value = false; if (objects.value.length === 1 && page.value > 1) page.value--; await load() } finally { deleting.value = false } }
  ...
  <section class="page arco-page">
    ...
    <a-card class="arco-filter-card" :bordered="false">
      <a-form :model="{}" layout="inline" @submit.prevent="search">
        ...
        <a-form-item><a-button type="primary" html-type="submit"><template #icon><IconSearch /></template>查询</a-button></a-form-item>
      </a-form>
    </a-card>
    <a-card class="arco-table-card" :bordered="false">
      <a-table :data="objects" :loading="loading" :pagination="false" row-key="id" :scroll="{ x: 1100 }" @row-click="openObject">
        <template #columns>
          ...
          <a-table-column title="操作" fixed="right" :width="140"><template #cell="{ record }"><div class="table-actions" @click.stop><a-button type="text" size="small" @click="router.push(`/objects/${record.id}/edit`)"><template #icon><IconEdit /></template>编辑</a-button><a-button type="text" status="danger" size="small" @click="askRemove(record)"><template #icon><IconDelete /></template>删除</a-button></div></template></a-table-column>
        </template>
        <template #empty><a-empty description="没有符合条件的对象" /></template>
      </a-table>
