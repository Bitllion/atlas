...
const route = useRoute(); const asset = ref<AssetDetail | null>(null); const events = ref<LifecycleEvent[]>([]); const types = ref<ObjectType[]>([]); const action = ref<'stock' | 'deploy' | null>(null)
...
const statusName: Record<string, string> = { RECEIVED: '已到货', STOCK: '库存中', ACTIVE: '使用中', DEPLOYED: '已部署', MAINTENANCE: '维护中', RETIRED: '已退役' }
function date(value: string | null | undefined) { return value ? new Date(value).toLocaleString('zh-CN') : '—' }
async function load() { const id = String(route.params.id); const [detail, lifecycle, objectTypes] = await Promise.all([assetApi.get(id), assetApi.lifecycle(id), objectApi.types()]); asset.value = detail.data; events.value = lifecycle.data.items; types.value = objectTypes.data.items }
